        ...
        <Box sx={{ p: 2, pb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <IconButton
            ...
            sx={{
              ...
              color: '#8E8E93',
              bgcolor: '#3A3B3C',
              '&:hover': { bgcolor: '#4A4B4C', color: '#fff' },
            }}
          >
        ...
        <List sx={{ flexGrow: 1, overflowY: 'auto', p: 1 }}>
          {isSearchingMode && searching && (
            <Box sx={{ display: 'flex', justifyContent: 'center', py: 2 }}>
              <CircularProgress size={20} sx={{ color: '#8E8E93' }} />
            </Box>
          )}
          {isSearchingMode && !searching && displayedConversations.length === 0 && (
            <Box sx={{ textAlign: 'center', py: 4 }}>
              <Typography sx={{ color: '#8E8E93', fontSize: '0.9rem' }}>Không tìm thấy hội thoại</Typography>
            </Box>
          )}
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', bgcolor: '#000000' }}>
        {activeConversationId ? (
          <>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box>
                  <Typography variant="h6" sx={{ fontWeight: 700, color: '#E4E6EB' }}>{getConversationName(activeConversation!)}</Typography>
                  <Typography variant="caption" sx={{ display: 'block', mt: -0.25, color: isUserOnline(getOtherParticipantId(activeConversation!)) ? '#44b700' : '#8E8E93' }}>
              ...
              {activeConversation?.type === 'GROUP' && (
                <IconButton
                  ...
                  sx={{ color: '#B0B3B8', '&:hover': { bgcolor: 'rgba(255,255,255,0.08)', color: '#fff' } }}
                >
                  <MoreVert />
                </IconButton>
              )}
                  ...
                  {Array.from({ length: MESSAGE_SKELETON_COUNT }).map((_, i) => (
                    <Box key={i} sx={{ display: 'flex', justifyContent: i % 3 === 0 ? 'flex-end' : 'flex-start' }}>
                      <Skeleton
                        ...
                        width={100 + ((i * 37) % 120)}
                        height={36}
                        sx={{ borderRadius: '18px', bgcolor: 'rgba(255,255,255,0.08)' }}
                      />
                    </Box>
                  ))}
                </Box>
                    ...
                    >
                      ...
                      >
                        {!isDeleted && (
                          <IconButton
                            ...
                            sx={{ opacity: 0, transition: 'opacity 0.15s', width: 26, height: 26, color: '#8E8E93', '&:hover': { color: '#fff', bgcolor: 'rgba(255,255,255,0.08)' } }}
                          >
                        ...
                        {isMediaOnly ? (
                          ...
                          <Paper
                            ...
                            sx={{
                              ...
                              bgcolor: isMe ? 'primary.main' : '#3A3B3C',
                              color: isMe ? '#fff' : '#E4E6EB',
                            }}
                          >
                            {replyQuote && (
                              <Box sx={{ borderLeft: '3px solid rgba(255,255,255,0.4)', pl: 1, mb: 0.5, opacity: 0.85 }}>
                                {replyQuote}
                              </Box>
                            )}
                            {renderMessageBody(msg)}
                      ...
                      {(isLastInGroup || seenAvatars.length > 0) && (
                        <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5, gap: 1, px: 0.5 }}>
                          {isLastInGroup && (
                            <Typography variant="caption" sx={{ color: '#6B6D70' }}>
                            ...
                            <Box sx={{ display: 'flex' }}>
                              {seenAvatars.slice(0, SEEN_AVATARS_VISIBLE_LIMIT).map((p, i) => (
                                <Avatar
                                  ...
                                  src={p.avatar}
                                  title={p.name}
                                  sx={{ width: 14, height: 14, border: '1px solid #0B0B0C', ml: i > 0 ? -0.75 : 0 }}
                                />
                              ))}
            ...
            {/* Input */}
            <Box sx={{ borderTop: '1px solid rgba(255,255,255,0.06)', bgcolor: 'rgba(255,255,255,0.02)' }}>
              {(replyTarget || editingMessageId) && (
                <Box sx={{ px: 2, pt: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderLeft: '3px solid', borderColor: 'primary.main', pl: 1 }}>
                    ...
                    {replyTarget && (
                      <Typography variant="caption" sx={{ color: '#8E8E93', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {getMessagePreviewText(replyTarget)}
                      </Typography>
                    )}
                  </Box>
                  <IconButton size="small" onClick={editingMessageId ? handleCancelEdit : handleCancelReply}>
                    <CloseIcon fontSize="small" sx={{ color: '#8E8E93' }} />
                  </IconButton>
                </Box>
              )}
              <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <IconButton
                  ...
                  sx={{ flexShrink: 0, color: '#8E8E93', '&:hover': { color: '#fff', bgcolor: 'rgba(255,255,255,0.08)' } }}
                >
                  {uploadingAttachment ? <CircularProgress size={18} sx={{ color: '#8E8E93' }} /> : <AttachFile />}
                </IconButton>
                <TextField
                  fullWidth
                  size="small"
                ...
                <IconButton
                  ...
                  sx={{
                    ...
                    height: 40,
                    flexShrink: 0,
                    bgcolor: inputText.trim() ? 'primary.main' : 'rgba(255,255,255,0.08)',
                    color: '#fff',
          ...
          >
            ...
            <Typography variant="h5" sx={{ color: '#E4E6EB', fontWeight: 700, mb: 0.5 }}>
              Chọn một hội thoại để bắt đầu
            </Typography>
            <Typography variant="body2" sx={{ color: '#6B6D70' }}>
