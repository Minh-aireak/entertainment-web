...
const WatchRoom: React.FC = () => {
  ...
  // Read inside the unmount cleanup below, which only depends on [roomId] - a ref keeps that
  // cleanup seeing the latest host status without re-registering (and firing) on every room
  // update, which a `room?.host` dependency would cause.
  ...
  useEffect(() => {
    ...
    roomService
      ...
      .then((res) => {
        if (!cancelled) setRoom(res.result);
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    return () => {
      if (roomId && !isHostRef.current) roomService.leaveRoom(roomId).catch(() => {});
    };
  }, [roomId]);
        ...
        >
          ...
          {isHost && (
            <Tooltip title={t('copyLink')}>
              <IconButton onClick={handleCopyInviteLink} sx={{ border: '1px solid rgba(255,255,255,0.1)' }}>
          ...
          {isHost ? (
            <Tooltip title="Đóng phòng">
              <IconButton onClick={handleCloseRoom} color="error" sx={{ border: '1px solid rgba(255,255,255,0.1)' }}>
                <CloseIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          ) : (
            <Tooltip title="Rời phòng">
              <IconButton onClick={handleLeave} sx={{ border: '1px solid rgba(255,255,255,0.1)' }}>
